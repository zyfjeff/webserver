...
"usage: %s [options]\n \
  -a,--address ADDR Bind to local address\n \
		(by default to all local address)\n \
  -h,--help    print this information.\n \
  -m, --module-dir DIR Load module form specified directory\n \
		(by default,use executable directory)\n \
  -p,--port PORT	Bind to specificed port	\n \
...
int main(int argc,char *argv[])
{
	...
	do {
		...
		case 'm':
			// check the module dir is exists? and check the file is dir? and check the dir mode
		  {
		...
		case 'p':
		  {
			// get the port and turn to the network byte order
			long value;
			char *end;
